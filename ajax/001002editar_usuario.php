...
if (empty($_POST['firstname2'])){
...
}  elseif (!empty($_POST['user_rut2']) && !validaRut($_POST['user_rut2'])) {
    $errors[] = 'Rut inválido.';
} elseif (
  ...
  //&& $_POST['user_consulta2']!='0' and $_POST['user_consulta2']!='1'
  ){
    ...
    $user_rut = mysqli_real_escape_string($con,(strip_tags($_POST["user_rut2"],ENT_QUOTES)));
    $user_email = mysqli_real_escape_string($con,(strip_tags($_POST["user_email2"],ENT_QUOTES)));
    $user_password_set = isset($_POST['user_password_set2']) && $_POST['user_password_set2'] == 'on' ? 1 : 0;
    $user_id=intval($_POST['mod_id']);
    $user_perfil_id=mysqli_real_escape_string($con,(strip_tags($_POST["user_idperfil2"],ENT_QUOTES)));

    $user_rut = str_replace('.', '', $user_rut);

    // write new user's data into database
    $sql = "UPDATE users SET firstname='".$firstname."', lastname='".$lastname."', user_name='".$user_name."', user_rut='".$user_rut."', user_email='".$user_email."', perfil_id='".$user_perfil_id."', user_password_set='".$user_password_set."'
