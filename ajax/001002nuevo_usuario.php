...
if (empty($_POST['firstname'])){
...
}  elseif (!empty($_POST['user_rut']) && !validaRut($_POST['user_rut'])) {
    $errors[] = 'Rut inválido.';
} elseif (
  ...
  //&& !empty($_POST['user_consulta'])
    ...
    $user_rut = mysqli_real_escape_string($con,(strip_tags($_POST["user_rut"],ENT_QUOTES)));
    $user_email = mysqli_real_escape_string($con,(strip_tags($_POST["user_email"],ENT_QUOTES)));
    $user_password_set = isset($_POST['user_password_set']) && $_POST['user_password_set'] == 'on' ? 1 : 0;
    $user_idperfil = mysqli_real_escape_string($con,(strip_tags($_POST["id_perfil"],ENT_QUOTES)));
    ...
    $sql = "INSERT INTO users (firstname,
            ...
            VALUES('".$firstname."','".$lastname."','" . $user_name ."','" . $user_rut . "', '" . $user_password_hash . "','" . $user_password_set . "','". $user_email . "','".$date_added."','".$user_idperfil."');";
    ...
    if ($query_new_user_insert) {
        ...
        $errors[] = "Lo sentimos , el registro falló. Por favor, regrese y vuelva a intentarlo.".mysqli_error($con).$sql;
    }
